...
- [x] **Cloud smoke tests** — weekly CI workflow (smoke.yml) with real API calls per provider: Gemini via GOOGLE_API_KEY, Codex via OPENAI_API_KEY, Ollama via Docker service container. Integration tests guarded by SMOKE_TEST env var
